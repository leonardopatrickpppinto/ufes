...
std::vector<carmen_robot_and_trailer_traj_point_t> dlite_path;
...
carmen_localize_ackerman_globalpos_message_handler(carmen_localize_ackerman_globalpos_message *msg)
{
	...
	if (final_goal_received != 0)
	{
		...
		dlite_path = carmen_path_planner_dlite_search(&initial_pose, &goal_pose, obstacle_distance_grid_map, goal_distance_map, nonholonomic_heuristic_cost_map);

		if (dlite_path.size() > 2)
		{
			printf("D-star-lite search time = %f\n", (double)(clock() - r_time) / CLOCKS_PER_SEC);

			//Precisa??
			if (USE_SMOOTH)
			{
				smooth_rddf_using_conjugate_gradient(dlite_path);
				printf("Full running time = %f\n", (double)(clock() - r_time) / CLOCKS_PER_SEC);
			}
			//Ainda não olhe esta função
			plan_path_poses = dlite_mount_offroad_planner_plan(&robot_position, final_goal, dlite_path);
			//Publica a mensagem
			publish_plan(plan_path_poses.path, msg);
	}
		double cont = 0.0;
		for (int i = 1; i< dlite_path.size(); i++)
			cont += DIST2D(dlite_path[i], dlite_path[i-1]);
...
void
carmen_path_planner_dlite_mapper_handler(carmen_mapper_map_message *message)
{
...
subscribe_messages()
{
	...
	carmen_mapper_subscribe_map_message(NULL, (carmen_handler_t) carmen_path_planner_dlite_mapper_handler, CARMEN_SUBSCRIBE_LATEST);
